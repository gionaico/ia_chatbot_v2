...
const MessageInput = ({ onSendMessage }:any) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="fixed bottom-0 left-0 w-full px-4 pb-4 message-input flex space-x-2 mx-auto">
  ...
  return (
    <div className="w-full chat-window p-4 bg-white rounded-lg shadow-lg ">
      {messages.map((msg:any, index:number) => (
        <div key={index} className={`message ${msg.sender === 'user' ? 'bg-white' : 'bg-gray-100'} p-2 my-2 rounded-md`}>
          {msg.text}
        </div>
      ))}
    </div>
...
export function Welcome() {
  ...
  return (
    <div className="flex flex-col gap-2 p-4 min-h-screen">
      ...
      <h1 className="text-center text-xl p-4">Chatbot</h1>
      <ChatWindow messages={[{sender:"user", text:"text1"},{sender:"chat",text:"messages2"}]} />
